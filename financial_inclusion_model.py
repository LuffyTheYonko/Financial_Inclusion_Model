...
enc = LabelEncoder()
xencoded = x.apply(enc.fit_transform)


if st.button("Predict"):
    result = data.predict(xencoded.values)
    if result == 1:
        st.write("Yes this person has a bank account")
    else:
        st.write("No this person does not have a bank account")
